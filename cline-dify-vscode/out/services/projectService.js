...
class ProjectService {
    ...
    async executeCommand() {
        ...
        if (this.isSensitiveCommand(command)) {
            const confirmation = await vscode.window.showWarningMessage('该命令可能修改或删除文件，确认继续执行吗？', { modal: true }, '继续执行');
    ...
    async showProjectStructure() {
        ...
        try {
            ...
            const panel = vscode.window.createWebviewPanel('projectStructure', 'Project Structure', vscode.ViewColumn.Beside, {});
    ...
    async readFileContent() {
        ...
        try {
            ...
            const panel = vscode.window.createWebviewPanel('fileContent', `File: ${filePath}`, vscode.ViewColumn.Beside, {
                enableScripts: false
            });
    ...
    getProjectStructureHtml(structure) {
        return `
        <!DOCTYPE html>
        <html lang="en">
        <head>
    ...
    getFileContentHtml(filePath, content) {
        return `
        <!DOCTYPE html>
        <html lang="en">
        <head>
    ...
    showCommandOutput(command, stdout, stderr) {
    ...
    buildProjectStructure(rootPath) {
        const lines = [];
        const rootName = path.basename(rootPath);
        lines.push(rootName);
        const traverse = (currentPath, prefix) => {
